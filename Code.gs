...
function quoteRecord_(data,existing) {
  ...
  record.fechaCreacion=record.fechaCreacion || date_(new Date());
  ...
  record.precioReal=clean_(data.precioReal)!=='' ? number_(data.precioReal) : number_(data.precioTotal); record.tipoDescuento=clean_(data.tipoDescuento) || 'porcentaje'; record.descuentoAplicado=Math.max(0,number_(data.descuentoAplicado)); record.precioFinalManual=clean_(data.precioFinalManual)!=='' ? number_(data.precioFinalManual) : '';
...
function saveQuoteTransaction_(data,forceConvert) {
  ...
  try {
    ...
    return {cotizacionId:record.cotizacionId,fechaCreacion:record.fechaCreacion,estatus:record.estatus,folioViaje:record.folioViaje || '',convertidoEnViaje:record.convertidoEnViaje};
  } finally { lock.releaseLock(); }
}
